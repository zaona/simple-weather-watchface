...
class PngQuant:
    ...
    def _convert_with_pillow(self, filename) -> bytes:
        if Image is None:
            raise BaseException(
                "pngquant not found and Pillow is not installed. "
                "Install pngquant or run `pip install pillow`.")

        with Image.open(filename) as im:
            im = im.convert("RGBA")
            alpha_min, alpha_max = im.getchannel("A").getextrema()
            has_alpha = alpha_min < 255 or alpha_max < 255
            dither = Image.FLOYDSTEINBERG if self.dither else 0
            if has_alpha:
                method = Image.FASTOCTREE if hasattr(Image, "FASTOCTREE") else Image.MEDIANCUT
                try:
                    quantized = im.quantize(colors=self.ncolors, method=method, dither=dither)
                except Exception:
            ...
            buf = io.BytesIO()
            quantized.save(buf, format="PNG", optimize=False)
            return buf.getvalue()
    ...
    def convert(self, filename) -> bytes:
        ...
        if shutil.which(self.executable) is None:
            return self._convert_with_pillow(filename)
        ...
        try:
            ...
            raise BaseException(
                "pngquant failed. Ensure pngquant is installed and works. "
                f"{output}".strip())
